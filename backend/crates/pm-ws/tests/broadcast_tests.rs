...
use common::{
    test_server::{create_test_server, TEST_JWT_SECRET},
...
async fn given_two_clients_same_tenant_when_broadcast_then_both_receive() {
    ...
    let sent_count = test_server.app_state.broadcaster
...
async fn given_fast_broadcasts_when_channel_full_then_handles_backpressure() {
    ...
    for i in 0..broadcast_count {
        ...
        let _ = test_server.app_state.broadcaster
            .broadcast(tenant_id, message)
            .await;
    }
    ...
    test_server.app_state.broadcaster
    ...
    for _ in 0..250 {  // More than we sent, to ensure we get to the final message
    ...
    assert!(received_final, "Client should still receive messages after backpressure");
...
async fn given_broadcast_when_client_disconnects_mid_receive_then_continues() {
    ...
    assert_eq!(sent_count, 1, "Should only broadcast to 1 remaining receiver");
...
async fn given_multiple_tenants_when_broadcast_to_one_then_others_unaffected() {
    ...
    let mut tenant1_client = WsTestClient::connect(
        &test_server.server,
        "tenant-1",
        "user-1",
        TEST_JWT_SECRET,
    ...
    let mut tenant2_client = WsTestClient::connect(
        &test_server.server,
        "tenant-2",
        "user-1",
        TEST_JWT_SECRET,
    ...
    let mut tenant3_client = WsTestClient::connect(
        &test_server.server,
        "tenant-3",
        "user-1",
        TEST_JWT_SECRET,
    ...
    assert!(tenant1_result.is_err(), "Tenant-1 should NOT receive message");

    let tenant3_result = timeout(timeout_duration, tenant3_client.receive_binary()).await;
    assert!(tenant3_result.is_err(), "Tenant-3 should NOT receive message");
