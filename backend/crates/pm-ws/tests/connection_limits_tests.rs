...
async fn given_tenant_at_limit_when_new_connection_then_rejected_503() {
    ...
    let _client1 = WsTestClient::connect(&test_server.server, tenant_id, "user-1", TEST_JWT_SECRET).await;
    let _client2 = WsTestClient::connect(&test_server.server, tenant_id, "user-2", TEST_JWT_SECRET).await;

    // When - Try to create 3rd connection for same tenant
    let response = test_server.server
        .get_websocket("/ws")
        .add_header("Authorization", format!("Bearer {}", create_test_token(tenant_id, "user-3", TEST_JWT_SECRET)))
        .await;
...
async fn given_server_at_total_limit_when_new_connection_then_rejected_503() {
    ...
    let _client1 = WsTestClient::connect(&test_server.server, "tenant-1", "user-1", TEST_JWT_SECRET).await;
    let _client2 = WsTestClient::connect(&test_server.server, "tenant-2", "user-1", TEST_JWT_SECRET).await;
    let _client3 = WsTestClient::connect(&test_server.server, "tenant-3", "user-1", TEST_JWT_SECRET).await;
    let _client4 = WsTestClient::connect(&test_server.server, "tenant-4", "user-1", TEST_JWT_SECRET).await;
    let _client5 = WsTestClient::connect(&test_server.server, "tenant-5", "user-1", TEST_JWT_SECRET).await;

    // When - Try to create 6th connection
    let response = test_server.server
        .get_websocket("/ws")
        .add_header("Authorization", format!("Bearer {}", create_test_token("tenant-6", "user-1", TEST_JWT_SECRET)))
        .await;
...
async fn given_tenant_at_limit_when_other_tenant_connects_then_succeeds() {
    ...
    let _client1 = WsTestClient::connect(&test_server.server, "tenant-1", "user-1", TEST_JWT_SECRET).await;
    let _client2 = WsTestClient::connect(&test_server.server, "tenant-1", "user-2", TEST_JWT_SECRET).await;

    // When - Tenant-2 tries to connect
    let client3 = WsTestClient::connect(&test_server.server, "tenant-2", "user-1", TEST_JWT_SECRET).await;
