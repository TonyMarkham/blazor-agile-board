...
### Sub-Process
```text
read `docs/session-plans/45-Session-Plan-Initial.md` and break it down into multiple sub-sessions targeting <50k tokens each that can be presented to a future user as teaching material. Take inspiration from the plan/sub-plans in `docs/session-plans/template/`
```
...
### Other
```text
Read `CRITICAL_OPERATING_CONSTRAINTS.md`, `docs/session-plans/50.4-Session-Plan.md`, `docs/session-plans/50-Session-Plan.md` and `docs/implementation-plan-v2.md`.

Please audit `docs/session-plans/50.4-Session-Plan.md` and cross reference it's contents against the current state of the project to avoid making poor assumptions.
...
```text
Builds clean and all tests pass. Please sanity check that everything in `docs/session-plans/50.4-Session-Plan.md` was implemented as expected.
```

### Update Docs
```text
Please update `docs/session-plans/50.4-Session-Plan.md` and `docs/session-plans/50-Session-Plan.md`
```
