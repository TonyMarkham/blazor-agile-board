...
grep -E "pub (use|fn|struct).*MessageValidator" backend/crates/pm-ws/src/lib.rs || echo "⚠️ MessageValidator not exported"
grep -E "pub (use|fn).*build_activity_log_created_event" backend/crates/pm-ws/src/lib.rs || echo "⚠️ build_activity_log_created_event not exported"
grep -E "pub (use|fn).*validate_hierarchy" backend/crates/pm-ws/src/lib.rs || echo "⚠️ validate_hierarchy not exported"
grep -E "pub (use|fn).*validate_status" backend/crates/pm-ws/src/lib.rs || echo "⚠️ validate_status not exported"
grep -E "pub (use|fn).*validate_priority" backend/crates/pm-ws/src/lib.rs || echo "⚠️ validate_priority not exported"
grep -E "pub (use|fn).*sanitize_string" backend/crates/pm-ws/src/lib.rs || echo "⚠️ sanitize_string not exported"
```
...
pub async fn delete_comment(
    ...
    let activity_clone = activity.clone();
    repo.soft_delete(comment_uuid, user_id).await?;
    ActivityLogRepository::create(&state.pool, &activity_clone).await?;
...
Add the REST API routes. Find the `build_router` function and add the API routes:
...
pub fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        // Add shared state
        .with_state(state)
        // CORS middleware (configurable via PM_API_RESTRICT_CORS env var)
...
- ✅ Success paths (create, update, list with filters)
- ✅ Validation errors (invalid type returns 400)
- ✅ Conflict errors (version mismatch returns 409)
- ✅ Business logic errors (deleting parent with children fails)
- ✅ Not found errors (invalid ID returns 404)
...
- ✅ Get with invalid UUID returns 400 (validation error)
...
- ✅ Work item existence is validated
- ✅ Empty lists return empty arrays
- ✅ Not found returns 404
- ✅ Soft delete works (comment not returned after deletion)
