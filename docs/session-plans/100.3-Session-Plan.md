...
- [ ] `cargo check -p pm-cli` passes
- [ ] `cargo build -p pm-cli` succeeds
- [ ] `cargo test -p pm-cli` passes (client + integration tests)
- [ ] `just build-rs-cli` works
- [ ] `./target/debug/pm --help` shows help
- [ ] `./target/debug/pm project list` returns JSON (with server running)

### Files Created (4)
- `backend/crates/pm-cli/Cargo.toml` (with wiremock dev-dependency)
- `backend/crates/pm-cli/src/main.rs`
- `backend/crates/pm-cli/src/client.rs` (with tests)
- `backend/crates/pm-cli/tests/cli_integration_tests.rs`

### Files Modified (2)
- `Cargo.toml` (root) - add wiremock to workspace.dependencies
- `justfile`
