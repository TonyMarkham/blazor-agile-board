# Session 120.1: Port File Module & Config Changes

**Parent Plan**: `120-Session-Plan.md`
**Target**: ~25-35k tokens
**Prerequisites**: `just check-backend && just test-backend` both pass
...
- [ ] `just test-rs-config` passes (all existing + new tests)
- [ ] `just check-backend` passes (workspace compile check)
- [ ] `just clippy-rs-config` passes

### Files Created (2)
- `pm-config/src/port_file.rs`
- `pm-config/src/tests/port_file.rs`

### Files Modified (5)
- `pm-config/Cargo.toml`
- `pm-config/src/lib.rs`
- `pm-config/src/server_config.rs`
- `pm-config/src/tests/mod.rs`
- `pm-config/src/tests/server.rs`
...
- Update config files to document `port = 0`
