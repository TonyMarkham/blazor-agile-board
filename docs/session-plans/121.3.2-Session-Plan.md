...
| **[121.3.2.5](121.3.2.5-Session-Plan.md)** | Time Entry CRUD | ~25k | — | **Complete** |
| **[121.3.2.6](121.3.2.6-Session-Plan.md)** | Bulk Sync Endpoints (export/import) | ~30k | 121.3.2.1–5 | **Complete** |
...
**Why last**: Reuses DTOs from the entity modules created in 121.3.2.1–5. No duplicate `sync_dto.rs` needed.

**Architecture Change**: DTOs and sync types moved to `pm-core` for better separation (core = domain types, pm-server = HTTP handlers only).

**Files Created (10):**
- `pm-core/src/sync/` - mod.rs, export_data.rs, import_result.rs, entity_import_counts.rs, sync_handlers.rs (helper functions)
- `pm-server/src/api/sync/` - mod.rs, export.rs, import.rs
- `pm-cli/src/sync_commands.rs` - CLI command definitions

**Files Modified (23):**
- 7 repositories: Add `find_all()` (project, sprint, swim_lane, work_item, comment, dependency, time_entry)
- 7 DTOs: Moved to pm-core, add `Deserialize` + `TryFrom` impls
- pm-core: models/mod.rs, lib.rs (exports)
- pm-server: api/mod.rs, lib.rs, routes.rs (integration)
- pm-cli: client/client.rs (2 methods), client/error.rs (Io variant), commands.rs, main.rs

**Key Learning**: WorkItemRepository kept as executor-based (not pool-based) because WebSocket handlers require transactions for atomicity.

**Testing**: `pm sync export --output data.json` and `pm sync import --file data.json` verified working with idempotent behavior.
