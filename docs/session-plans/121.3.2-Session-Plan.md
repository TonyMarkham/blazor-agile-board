...
|---------|-------|-------------|------------|--------|
| **[121.3.2.1](121.3.2.1-Session-Plan.md)** | Project Create/Update/Delete | ~25k | — | Pending |
| **[121.3.2.2](121.3.2.2-Session-Plan.md)** | Sprint CRUD | ~30k | — | Pending |
| **[121.3.2.3](121.3.2.3-Session-Plan.md)** | Dependency Create/Read/Delete | ~20k | — | Pending |
| **[121.3.2.4](121.3.2.4-Session-Plan.md)** | Swim Lane CRUD | ~25k | — | Pending |
...
|--------|:--------:|:---------:|:--------------:|:-------------:|:----------------:|
| **Projects** | list, get | -- | GET x2 only | Full CRUD | Yes |
| **Work Items** | full | full | Full CRUD | Full CRUD | Yes |
| **Comments** | list | full | Full CRUD | Full CRUD | Yes |
| **Sprints** | -- | -- | **None** | Full CRUD | Yes (empty `[]`) |
| **Dependencies** | -- | -- | **None** | Full CRUD | **MISSING** |
| **Swim Lanes** | -- | -- | **None** | Full CRUD | Yes (empty `[]`) |
...
**Files Created:**
- `pm-server/src/api/swim_lanes/` - mod.rs, swim_lane_dto.rs, swim_lane_response.rs, swim_lane_list_response.rs, create_swim_lane_request.rs, update_swim_lane_request.rs, swim_lanes.rs
- `pm-cli/src/swim_lane_commands.rs` - CLI command definitions
...
- `pm-cli/src/main.rs` - Wire swim lane commands
...
## Architectural Note: WebSocket Broadcasts

Existing mutation handlers in `work_items.rs` and `comments.rs` broadcast via WebSocket (`state.registry.broadcast_to_project()` + `broadcast_activity_log_created()`). The new REST endpoints in sessions 121.3.2.1–5 intentionally **omit** WebSocket broadcasts. These endpoints exist for CLI and sync tooling only — real-time updates remain exclusive to the WebSocket protocol handlers in `pm-ws`. Adding broadcasts would duplicate the WebSocket handler logic and create two code paths for the same mutations.

**Consequence**: REST mutations do **not** create `pm_activity_log` entries. The activity log is written by the WebSocket broadcast path. CLI/sync operations are therefore "silent" in the audit trail. This is acceptable for the sync use case (which replaces the entire dataset) but worth noting for individual CRUD via CLI.

Validation in handlers should use `sanitize_string()` (from `pm_ws`, XSS prevention) for user-supplied strings, and construct `ApiError::Validation` for invalid inputs. See `work_items.rs` lines 181-182 for the pattern.
...
| 121.3.2.4 | 8 files (swim lane module + CLI) |
...
cargo run -p pm-cli -- swim-lane --help
