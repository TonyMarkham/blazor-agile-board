...
Update `handle_create_dependency` to call the shared method instead of the local function:

```rust
// Replace: detect_circular_dependency(&repo, blocking_id, blocked_id).await?;
// With:
if let Some(cycle_path) = repo.detect_cycle(blocking_id, blocked_id).await
    .map_err(|e| WsError::Database { message: e.to_string(), ... })?
{
...
The local `detect_circular_dependency()` function (lines 52-119) can then be removed.
...
- Validate both work item IDs exist (via `WorkItemRepository::find_by_id()`)
...
- `DependencyRepository::delete()` (soft delete)
- Return `DeleteResponse`

**Note**: These REST endpoints are for CLI/sync tooling only — no WebSocket broadcasts. Cycle detection uses the shared `DependencyRepository::detect_cycle()` method (extracted in Step 0), so both REST and WebSocket paths enforce the same validation.
...
**Session 121.3.2.4** adds Swim Lane CRUD (REST + CLI).
