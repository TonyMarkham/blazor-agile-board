...
- Release profile optimizes for binary size
...
**Key Teaching Points:**
- Each error variant includes all context needed for debugging
- Recovery hints are user-facing, not developer-facing
...
- [ ] Directory structure created: `desktop/src-tauri/src/server/`
- [ ] `cargo check` passes in `desktop/src-tauri`
- [ ] All files created:
  - [ ] `desktop/src-tauri/Cargo.toml`
  - [ ] `desktop/src-tauri/build.rs`
  - [ ] `desktop/src-tauri/src/server/mod.rs`
  - [ ] `desktop/src-tauri/src/server/error.rs`
  - [ ] `desktop/src-tauri/src/server/config.rs`
  - [ ] `desktop/src-tauri/src/server/port.rs`
  - [ ] `desktop/src-tauri/src/server/lock.rs`

### Files Created (7)

| File | Purpose | Lines |
|------|---------|-------|
| `Cargo.toml` | Dependencies and build config | ~60 |
| `build.rs` | Tauri build hook | ~3 |
| `src/server/mod.rs` | Module exports | ~20 |
| `src/server/error.rs` | Error types with recovery hints | ~120 |
| `src/server/config.rs` | Versioned configuration | ~300 |
| `src/server/port.rs` | Port allocation | ~60 |
| `src/server/lock.rs` | Single-instance lock | ~130 |
...
1. **Error Design Pattern**: Errors should be specific, actionable, and distinguish transient from permanent failures

2. **Configuration Versioning**: Always version your config format to support smooth upgrades

3. **Atomic File Writes**: Write to temp file, then rename - prevents partial writes

4. **Cross-Platform Code**: Use `#[cfg(unix)]` and `#[cfg(windows)]` for platform-specific behavior

5. **Lock Files**: Essential for single-instance desktop apps to prevent database corruption
