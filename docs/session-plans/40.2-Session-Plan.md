...
3. **Backend Health Endpoints** - Required endpoints in pm-server
...
### Step 1: Create Health Status Types
...
### Step 2: Create Server State Enum

Add to the health.rs file or create in lifecycle.rs:
...
### Step 3: Update Server Module Exports
...
### Step 4: Add Backend Health Endpoints

**Modify**: `backend/pm-server/src/main.rs`

Add these health endpoints to the pm-server:
...
async fn shutdown_handler(
    ...
    tracing::info!("Graceful shutdown requested via HTTP");
...
**Note**: The exact integration depends on your existing pm-server structure. The key endpoints needed are:
- `GET /health` - Basic liveness check
- `GET /ready` - Readiness check with database probe
- `POST /admin/checkpoint` - Force WAL checkpoint
...
- [ ] Health endpoints added to pm-server
- [ ] Files created/modified:
  - [ ] `desktop/src-tauri/src/server/health.rs`
  - [ ] `desktop/src-tauri/src/server/lifecycle.rs`
  - [ ] `desktop/src-tauri/src/server/mod.rs` (updated)
  - [ ] `backend/pm-server/src/main.rs` (updated with health endpoints)

### Files Created (2)

| File | Purpose | Lines |
|------|---------|-------|
| `src/server/health.rs` | Health monitoring | ~150 |
| `src/server/lifecycle.rs` | Process lifecycle | ~400 |

### Files Modified (2)

| File | Change |
|------|--------|
| `src/server/mod.rs` | Export health, lifecycle modules |
| `pm-server/src/main.rs` | Add health endpoints |
