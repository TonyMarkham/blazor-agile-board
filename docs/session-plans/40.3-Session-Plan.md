...
- [ ] `cargo build` passes in `desktop/src-tauri`
- [ ] `cargo tauri dev` launches (with placeholder frontend)
...
| File | Purpose | Lines |
|------|---------|-------|
| `tauri.conf.json` | Tauri configuration | ~90 |
| `src/commands.rs` | IPC command handlers | ~200 |
| `src/tray.rs` | System tray manager | ~150 |
| `src/logging.rs` | Log rotation setup | ~50 |
| `src/lib.rs` | Application entry point | ~100 |
| `src/main.rs` | Binary entry point | ~10 |
