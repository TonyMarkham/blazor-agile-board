...
**Create**: `frontend/ProjectManagement.Core/Validation/ValidationResult.cs`
...
**Create**: `desktop/src-tauri/src/identity/mod.rs`
...
use crate::handlers::connection::{extract_user_id, ConnectionError};
use std::collections::HashMap;
use uuid::Uuid;

#[test]
fn given_auth_enabled_and_user_id_param_when_extract_then_security_violation() {
...
- [ ] `cd desktop && cargo test` passes
- [ ] `cd desktop && cargo tauri build --debug` succeeds

### Files Created (8)
- `frontend/ProjectManagement.Core/Models/UserIdentity.cs`
- `frontend/ProjectManagement.Core/Validation/ValidationResult.cs`
- `desktop/src-tauri/src/identity/mod.rs` - Identity module with load/save logic
- `desktop/src-tauri/src/identity/error.rs` - Structured error types
- `desktop/src-tauri/src/tests/mod.rs` - Test module declaration
- `desktop/src-tauri/src/tests/identity.rs` - Identity unit tests
- `backend/crates/pm-ws/src/handlers/connection.rs`
- `backend/crates/pm-ws/src/tests/connection.rs` - Connection unit tests

### Files Modified (3)
- `desktop/src-tauri/src/lib.rs` - Add `mod identity;` and `#[cfg(test)] mod tests;`
- `desktop/src-tauri/src/commands.rs` - Add identity commands
- `backend/crates/pm-ws/src/tests/mod.rs` - Add `mod connection;`
