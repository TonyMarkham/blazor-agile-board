...
**Create**: `frontend/ProjectManagement.Components/Sprints/SprintCard.razor`

**Note**: Folder named `Sprints/` (plural) to avoid namespace collision with `Sprint` model type.
...
**Create**: `frontend/ProjectManagement.Components/Sprints/SprintCard.razor.css`
...
**Create**: `frontend/ProjectManagement.Components/Sprints/SprintDialog.razor`
...
**Note**: This project registers services directly in `Program.cs` rather than using a separate `DependencyInjection.cs` file.

**Find the State Management section (around line 71-75) and add:**
```csharp
// === State Management ===
...
builder.Services.AddSingleton<ICommentStore, CommentStore>();  // <-- Add this line
...
- [x] `just test-frontend` passes ✅ (365 tests passing)
- [x] `just check` passes ✅

### Files Created (9)
- `ProjectManagement.Core/Interfaces/ICommentStore.cs` (47 lines)
- `ProjectManagement.Services/State/CommentStore.cs` (235 lines)
- `ProjectManagement.Components/Sprints/SprintCard.razor` (95 lines)
- `ProjectManagement.Components/Sprints/SprintCard.razor.css` (42 lines)
- `ProjectManagement.Components/Sprints/SprintDialog.razor` (168 lines)
- `ProjectManagement.Components/Comments/CommentList.razor` (106 lines)
- `ProjectManagement.Components/Comments/CommentList.razor.css` (45 lines)
- `ProjectManagement.Components/Comments/CommentEditor.razor` (52 lines)
- `ProjectManagement.Components/Comments/CommentEditor.razor.css` (14 lines)

### Files Modified (1)
- `ProjectManagement.Wasm/Program.cs` (1 line added for ICommentStore registration)
...
**What was delivered:**
- ✅ ICommentStore interface with full CRUD operations
- ✅ CommentStore with WebSocket integration and optimistic updates
- ✅ SprintCard component with status-based styling and actions
- ✅ SprintDialog component with form validation and status transitions
- ✅ CommentList component with inline editing and author-only permissions
- ✅ CommentEditor component with character counting and keyboard shortcuts
- ✅ Service registration in Program.cs (Singleton lifetime)
- ✅ All builds clean (0 warnings, 0 errors)
- ✅ All 365 tests passing

**Adjustments from original plan:**
1. **Folder naming**: Used `Sprints/` (plural) instead of `Sprint/` to avoid namespace collision with `Sprint` model type
2. **Service registration**: Added to `Program.cs` instead of `DependencyInjection.cs` (which doesn't exist in this project structure)

Both adjustments follow existing codebase patterns and improve code quality.
