...
3. **Frontend Converter Tests** - Unit tests for ProtoConverter
4. **Frontend Store Tests** - Unit tests for SprintStore and CommentStore
...
use pm_proto::{
    CreateSprintRequest, UpdateSprintRequest, DeleteSprintRequest, GetSprintsRequest,
...
impl TestFixture {
    async fn new() -> Self {
        ...
        sqlx::query!(
            r#"
            INSERT INTO pm_work_items (id, item_type, project_id, position, title, status, priority, version, created_at, updated_at, created_by, updated_by)
            VALUES (?, 'project', ?, 1, 'Test Project', 'active', 'medium', 1, ?, ?, ?, ?)
            "#,
...
### Step 5: Create SprintStore Tests
...
### Files Created (5)
- `pm-ws/tests/sprint_handler_tests.rs`
- `pm-ws/tests/comment_handler_tests.rs`
- `ProjectManagement.Core.Tests/Converters/SprintConverterTests.cs`
- `ProjectManagement.Core.Tests/Converters/CommentConverterTests.cs`
- `ProjectManagement.Services.Tests/State/SprintStoreTests.cs`
...
1. **Proto Schema** (50.1) - Protobuf messages for sprints and comments
2. **Backend Handlers** (50.1-50.2) - Sprint/Comment CRUD with authorization
3. **Frontend Integration** (50.3) - WebSocket client and proto converters
4. **UI Components** (50.4) - Sprint cards, dialogs, comment list/editor
5. **Testing** (50.5) - Comprehensive backend and frontend tests
