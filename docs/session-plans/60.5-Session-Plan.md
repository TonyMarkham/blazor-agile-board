...
- [ ] `just test-rs-ws` passes (new backend tests)
- [ ] `just test-backend` passes (all backend tests)
- [ ] `just test-cs-core` passes (new converter tests)
- [ ] `just test-cs-services` passes (new store tests)
- [ ] `just test-frontend` passes (all frontend tests)
- [ ] `just test` passes (everything)

### Files Created (6)

| File | Purpose |
|------|---------|
| `time_entry_handler_tests.rs` | 10 backend time entry tests |
| `dependency_handler_tests.rs` | 10 backend dependency tests |
| `TimeEntryConverterTests.cs` | Proto conversion tests |
| `DependencyConverterTests.cs` | Proto conversion tests |
| `TimeEntryStoreTests.cs` | Store unit tests with mocks |
| `DependencyStoreTests.cs` | Store unit tests with mocks |
