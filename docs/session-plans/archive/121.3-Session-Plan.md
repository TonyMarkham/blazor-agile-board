...
Rust identifies platforms using "target triples" like `aarch64-apple-darwin` (Apple Silicon Mac), `x86_64-unknown-linux-gnu` (Linux x64), or `x86_64-pc-windows-msvc` (Windows x64). Using the target triple in archive names is the standard convention for Rust projects (used by rustup, cargo-dist, and most Rust release tooling).

The `just` tool supports backtick expressions that run shell commands at parse time. Rust provides `rustc --print host-tuple` which outputs the host target triple directly without parsing.

**Cross-Platform Improvements:**
- **Target detection**: Use `rustc --print host-tuple` instead of `rustc -vV | grep host | cut -d' ' -f2` (works in cmd.exe, PowerShell, Git Bash)
- **Version extraction**: Use `os_family()` conditional to auto-extract version from `Cargo.toml` on all platforms
