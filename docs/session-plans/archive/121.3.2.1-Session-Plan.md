...
- Create `ActivityLog::created("project", project.id, user_id)`
- **Transaction**: `ProjectRepository::create()` + `ActivityLogRepository::create()` + commit
- **Broadcast**: `broadcast_activity_log_created(project_id, None, None, ...)` (non-fatal)
- **Broadcast**: `broadcast_to_project(project_id, ...)` with entity event (non-fatal)
...
- Create `ActivityLog::updated("project", project.id, user_id, &[])`
- **Transaction**: `ProjectRepository::update()` + `ActivityLogRepository::create()` + commit
- **Broadcast**: `broadcast_activity_log_created(project_id, None, None, ...)` (non-fatal)
- **Broadcast**: `broadcast_to_project(project_id, ...)` with entity event (non-fatal)
- Return `ProjectResponse`
...
- Load existing project via `find_by_id()` — return 404 if not found
- Create `ActivityLog::deleted("project", project.id, user_id)`
- **Transaction**: `ProjectRepository::delete()` + `ActivityLogRepository::create()` + commit
- **Broadcast**: `broadcast_activity_log_created(project_id, None, None, ...)` (non-fatal)
- **Broadcast**: `broadcast_to_project(project_id, ...)` with entity event (non-fatal)
- Return `DeleteResponse`

**Required**: All mutation handlers (create, update, delete) MUST write `ActivityLog` entries and broadcast via WebSocket, matching the established pattern in `work_items.rs`:
1. Create `ActivityLog::created("project", ...)` / `::updated(...)` / `::deleted(...)`
2. Write entity + activity log in same transaction
3. Broadcast `ActivityLogCreated` via `state.registry.broadcast_activity_log_created(project_id, None, None, message)` (non-fatal)
4. Broadcast entity event via `state.registry.broadcast_to_project(project_id, message)` (non-fatal)

Use `build_activity_log_created_event()` for the activity broadcast. For entity broadcasts, use the project-specific response builders from `pm_ws` (if they exist) or build a generic project response.

Add necessary imports: `CreateProjectRequest`, `UpdateProjectRequest`, `UserId`, `DeleteResponse`, `sanitize_string`, `build_activity_log_created_event` (from `pm_ws`), `ActivityLog` (from `pm_core`), `ActivityLogRepository` (from `pm_db`), `Message` (from `axum::extract::ws`), `ProstMessage` (from `prost`).
...
Commands::Project { action } => match action {
    ...
    ProjectCommands::Update { id, title, description, status, expected_version } => {
        client
            .update_project(&id, title.as_deref(), description.as_deref(), status.as_deref(), expected_version)
            .await
    }
...
**Token Usage**: ~87k tokens (teaching mode)
**Verification**: All checks pass, all tests pass (23 total), manual CLI testing successful

### Implementation Notes

**CLI Flag Naming Deviation:**
- The plan originally specified `--version` for the Update command's optimistic locking field
- **Changed to `--expected-version`** to avoid conflict with clap's auto-generated `--version` flag (which displays CLI version)
- This is the correct and necessary solution
- User command: `pm project update <id> --expected-version 1 --status archived`

### What Was Built

**REST API:**
- `POST /api/v1/projects` - Create with uniqueness validation
- `PUT /api/v1/projects/{id}` - Update with optimistic locking
- `DELETE /api/v1/projects/{id}` - Soft delete

**CLI Commands:**
- `pm project create --title "X" --key "PROJ" [--description "Y"]`
- `pm project update <id> [--title] [--description] [--status] --expected-version N`
- `pm project delete <id>`

**Patterns Followed:**
- ✅ WebSocket broadcasts (activity log + entity events)
- ✅ Transactions (entity + activity log atomically)
...
**Modified (7):**
- `backend/pm-server/src/api/projects/projects.rs` (3 handlers)
- `backend/pm-server/src/api/projects/mod.rs` (module declarations)
- `backend/pm-server/src/lib.rs` (exports)
- `backend/pm-server/src/routes.rs` (3 routes)
- `backend/crates/pm-cli/src/client/client.rs` (3 methods)
- `backend/crates/pm-cli/src/project_commands.rs` (3 variants)
- `backend/crates/pm-cli/src/main.rs` (command wiring)

**Also Required:**
- `backend/crates/pm-ws/src/lib.rs` - Exported `build_project_*_response` functions (these existed but weren't exported)
