...
After 4+ debugging sessions, the root cause was **NOT** Blazor, Radzen, or HTML5 drag-drop APIs.

**Root Cause:** Tauri's `dragDropEnabled` (default: `true`) intercepts HTML5 drag events at the native OS level to handle file drops. This prevents drag events from reaching the browser's HTML5 drag-drop API that Radzen depends on.

### What Actually Happened

The planned 7-step migration to Radzen was **partially** correct in approach but solving the wrong problem. The real fix was a **single Tauri config change**.

#### Changes Made

1. **Tauri Configuration** (`desktop/src-tauri/tauri.conf.json`)
   ```json
   "windows": [
     {
       "title": "Project Manager",
       ...
       "dragDropEnabled": false  // <-- THIS WAS THE FIX
     }
   ]
   ```

2. **Backend Status Validation** (`backend/crates/pm-ws/src/handlers/work_item.rs`)
   - Added "backlog" to valid statuses (was missing, causing Backlog column drops to fail)

3. **KanbanBoard Restructure** (`frontend/.../KanbanBoard.razor`)
   - Wrapped columns with `RadzenDropZoneContainer`
   - Each `KanbanColumn` contains `RadzenDropZone`
   - Cards rendered via `Template` parameter
   - Preserved filters, accessibility, and connection state handling

4. **Backend Tests Updated** (`backend/crates/pm-ws/src/tests/property_tests.rs`)
   - Added "backlog" to test cases

### Steps NOT Needed

Most of the planned CSS changes, ItemRender callbacks, and complex state management were **not required**. The existing Radzen setup worked fine once Tauri stopped intercepting the events.
...
| `backend/crates/pm-ws/src/tests/property_tests.rs` | Updated tests |
| `frontend/.../WorkItems/KanbanBoard.razor` | Integrated RadzenDropZoneContainer |
| `frontend/.../WorkItems/KanbanColumn.razor` | Contains RadzenDropZone (already had it) |
| `frontend/.../WorkItems/KanbanCard.razor` | Used as-is via Template |

### Key Learnings

1. **Tauri + HTML5 Drag-Drop:** Any web app using HTML5 drag-drop inside Tauri must set `dragDropEnabled: false`. This is NOT documented in Radzen's docs because it's Tauri-specific.

2. **Test in Browser First:** When debugging webview issues, test standalone in browser to isolate platform vs. code problems.

3. **Check Full Stack:** Frontend drag-drop can appear "broken" when backend rejects status values. Always verify request/response flow.

4. **Radzen FAQ Gap:** Radzen should document common webview environments (Tauri, Electron) where drag-drop requires special configuration.

### Verification

All original verification criteria passed:
- ✅ Drag card from Backlog → To Do → card moves
- ✅ All swimlanes accept drops (including Backlog after backend fix)
- ✅ Visual feedback during drag
- ✅ Drop in same column = no-op
- ✅ Tests pass
- ✅ Filters work
