...
- [x] `just test-frontend` passes (existing tests)
...
| `Models/Dependency.cs` | Domain model for dependency relationships |
| `Models/DependencyType.cs` | Enum for Blocks/RelatesTo (separated for clarity) |
| `Models/StartTimerRequest.cs` | Request DTO for starting a timer |
| `Models/CreateTimeEntryRequest.cs` | Request DTO for manual time entry |
| `Models/UpdateTimeEntryRequest.cs` | Request DTO for updating time entry |
| `Models/CreateDependencyRequest.cs` | Request DTO for creating dependency |

**Note**: Request DTOs were split into individual files instead of grouped files (`TimeEntryRequests.cs`, `DependencyRequests.cs`). This matches the existing pattern in the codebase where each request type has its own file.

### Files Modified (4)

| File | Change |
|------|--------|
| `Converters/ProtoConverter.cs` | Added 6 conversion methods (TimeEntry ↔ Proto, Dependency ↔ Proto, enum conversions) |
| `WebSocket/IWebSocketClient.cs` | Added 10 operations + 7 events for Time Entry & Dependency |
| `WebSocket/WebSocketClient.cs` | Implemented event declarations, 10 operations, 7 broadcast handlers |
| `Resilience/ResilientWebSocketClient.cs` | Added event pass-throughs + operation wrappers for resilience layer |
...
Proto uses different patterns for optional fields depending on type:

**Scalar fields (int, long, string)** - Use `HasXxx` pattern:
```csharp
EndedAt = proto.HasEndedAt
    ? DateTimeOffset.FromUnixTimeSeconds(proto.EndedAt).UtcDateTime
    : null,
...
**Message fields (objects)** - Use null checks directly:
```csharp
...
This is because protobuf generates `HasXxx` properties only for optional scalar fields, not for optional message types.
