...
4. **ProtoConverter** (Step 17) - Map new fields including `update_parent`, CREATE converter for UpdateWorkItemRequest
5. **WorkItemViewModel** (Step 18) - Expose new properties
6. **Unit Tests** (Step 19) - Test edge cases and proto conversion
7. **WebSocketClient** (Step 20 - CRITICAL) - Wire up ParentId/UpdateParent fields in UpdateWorkItemAsync
...
#### 17.4 CREATE `ToProto(UpdateWorkItemRequest)` (NEW - CRITICAL FIX)

**IMPORTANT**: This method doesn't currently exist in ProtoConverter.cs. We need to CREATE it for consistency with other request converters (Sprint, Comment have similar methods). Currently, WebSocketClient.UpdateWorkItemAsync manually maps fields, but we'll centralize this logic here.

**Add this new method to ProtoConverter.cs** (in the WorkItem Conversions region, after `ToProto(DomainWorkItem)`):
...
### Step 20: Update WebSocketClient to Use Converter (CRITICAL)

**File**: `frontend/ProjectManagement.Services/WebSocket/WebSocketClient.cs`

**CRITICAL FIX**: The plan originally missed this file. WebSocketClient.UpdateWorkItemAsync manually constructs proto messages instead of using ProtoConverter. We need to refactor it to use the new converter method from Step 17.4.

**Find this code** (around lines 365-393):
...
var message = new Pm.WebSocketMessage
{
    MessageId = Guid.NewGuid().ToString(),
    Timestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds(),
    UpdateWorkItemRequest = ProtoConverter.ToProto(request)  // Use the new converter!
};
```

**Why this matters:**
- The original code manually mapped 8+ fields
- It was missing ParentId and UpdateParent handling
- Using ProtoConverter centralizes the logic and prevents future bugs
- Matches the pattern used for Sprint/Comment updates

**Add required using** at the top of WebSocketClient.cs:
...
- [ ] **Step 17**: ProtoConverter updated (including **`update_parent` mapping** and new converter method)
- [ ] **Step 18**: WorkItemViewModel updated
- [ ] **Step 19**: Unit tests added for null safety and proto conversion
- [ ] **Step 20**: WebSocketClient refactored to use ProtoConverter (CRITICAL)
...
- ✅ **Converter created**: New ToProto(UpdateWorkItemRequest) method for consistency
- ✅ **WebSocketClient refactored**: Uses ProtoConverter instead of manual mapping (prevents bugs)
...
- `ProjectManagement.Core/Converters/ProtoConverter.cs` - Map new fields, **CREATE** `ToProto(UpdateWorkItemRequest)`
- `ProjectManagement.Core/ViewModels/WorkItemViewModel.cs` - Expose `ItemNumber`, `GetDisplayKey()`
- `ProjectManagement.Services/WebSocket/WebSocketClient.cs` - **Refactor to use ProtoConverter** (CRITICAL)
